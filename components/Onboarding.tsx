...
const StepIndicator: React.FC<{step: number, currentStep: number}> = ({ step, currentStep }) => (
    <div className="flex items-center gap-2">
        <div className={`w-8 h-8 rounded-full flex items-center justify-center font-bold text-lg transition-colors ${currentStep >= step ? 'bg-blue-600 text-white' : 'bg-slate-700 text-slate-400'}`}>
            {step}
        </div>
    </div>
);
        ...
        <div className="bg-slate-800/60 border border-slate-700 rounded-2xl shadow-2xl backdrop-blur-lg p-8">
            <div className="flex justify-center items-center gap-4 mb-8">
                ...
                <div className="flex-1 h-0.5 bg-slate-700">
                    <div className="h-full bg-blue-600 transition-all duration-300" style={{ width: step === 1 ? '0%' : '100%' }}></div>
                </div>
            ...
            <form onSubmit={handleSubmit}>
                <div style={{ display: step === 1 ? 'block' : 'none' }}>
                    <h2 className="text-2xl font-semibold mb-2 text-blue-300">Step 1: Your Interests & Goals</h2>
                    <p className="text-slate-400 mb-6">What are you passionate about? What role are you aiming for?</p>
                    
                    <div className="space-y-4">
                        <div>
                            <label htmlFor="targetRole" className="block text-sm font-medium text-slate-300 mb-1">What's your dream job title?</label>
                            <input
                                id="targetRole"
                                type="text"
                                value={targetRole}
                                onChange={(e) => setTargetRole(e.target.value)}
                                className="w-full bg-slate-900/70 border border-slate-700 rounded-md p-2.5 focus:ring-2 focus:ring-blue-500 focus:outline-none"
                                placeholder="e.g., Full-Stack Developer, Data Scientist"
                            />
                        </div>
                         <div>
                            <label htmlFor="interests" className="block text-sm font-medium text-slate-300 mb-1">List your tech interests (comma-separated).</label>
                            <input
                                id="interests"
                                type="text"
                                value={interests}
                        ...
                        <div>
                            <label htmlFor="githubUrl" className="block text-sm font-medium text-slate-300 mb-1">GitHub Profile (Optional)</label>
                            <input
                                id="githubUrl"
                                type="url"
                                value={githubUrl}
                    ...
                    <button type="button" onClick={() => setStep(2)} className="w-full mt-8 px-6 py-3 font-semibold text-white bg-blue-600 rounded-lg hover:bg-blue-700 transition-colors shadow-md">
                        Next Step
                    </button>
                </div>
                
                <div style={{ display: step === 2 ? 'block' : 'none' }}>
                     <h2 className="text-2xl font-semibold mb-2 text-blue-300">Step 2: Your Experience</h2>
                    ...
                    <div className="flex gap-4 mt-8">
                        ...
                        <button type="submit" disabled={isLoading} className="w-full inline-flex items-center justify-center gap-2 px-6 py-3 font-semibold text-white bg-gradient-to-r from-purple-600 to-indigo-600 rounded-lg hover:opacity-90 disabled:opacity-50 transition-opacity shadow-lg">
                            <SparklesIcon />
